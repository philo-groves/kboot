//! Ported from https://github.com/rust-osdev/bootloader/blob/main/src/fat.rs
...
pub fn create_fat_filesystem_image(files: BTreeMap<Cow<'static, str>, FileDataSource>, internal_data: BTreeMap<String, FileDataSource>) -> anyhow::Result<NamedTempFile> {
...
pub fn add_files_to_image(
    ...
    for (target_path_raw, source) in files {
        // Convert Windows backslashes to forward slashes for FAT filesystem
        let normalized_path = target_path_raw.replace('\\', "/");
        ...
        for ancestor in ancestors.into_iter().rev().skip(1) {
            let ancestor_str = ancestor.display().to_string().replace('\\', "/");
            root_dir
                .create_dir(&ancestor_str)
                .with_context(|| {
                    format!(
